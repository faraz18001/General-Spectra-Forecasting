...
async def run_simulation_endpoint(request: SimulationRequest):
    ...
    print("=== /api/simulate incoming ===")
    print(f"num_trials={request.num_trials}, inflow_type={request.inflow_type}")
    print(f"services={request.services}")
    print(f"workgroups count={len(request.workgroups)}")
    if request.calling_profiles:
        for cp in request.calling_profiles:
            order_preview = "|".join([",".join(item.category for item in level) for level in cp.order])
            print(f"  profile id={cp.id} name={cp.name} type={cp.type} order=[{order_preview}]")
    print(f"counters={request.counters}")
    print(f"resolution_mode={request.resolution_mode}")
    ...
    try:
        ...
        print(
            f"Simulation result: served={results.get('summary', {}).get('total_customers_served')}, "
            f"balked={results.get('summary', {}).get('total_customers_balked')}, "
            f"avg_wait={results.get('summary', {}).get('avg_wait_time_mins')}, "
            f"util={results.get('summary', {}).get('avg_teller_utilization_pct')}"
        )
...
@app.get("/api/simulation/eqms-config")
async def get_eqms_config(branch_id: int = None):
    """Fetch the branch's eQMS configuration (services, calling profiles, counters).

    The frontend passes the eQMS branch ID directly. The previous implementation
    used branch_name to look up the ID from ICA_BranchMappingNew, which was an
    ICA client-specific table that does not exist on other deployments.
    """
    if not branch_id:
        raise HTTPException(status_code=400, detail="branch_id query parameter is required")
    from eqms_config import get_eqms_config
    config = get_eqms_config(branch_id)
    if "error" in config:
