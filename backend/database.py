...
class SimulationSetting(Base):
    ...
    and links to child tables for services, hourly inflows, workgroups,
    calling profiles, and counters.
    """
    ...
    resolution_mode = Column(String(50), nullable=True, default="hybrid")
    category_max_wait_times_json = Column(Text, nullable=True)
    created_at = Column(DateTime, default=datetime.utcnow)
    ...
    calling_profiles = relationship("SimulationCallingProfile", back_populates="setting", cascade="all, delete-orphan")
    counters = relationship("SimulationCounter", back_populates="setting", cascade="all, delete-orphan")
...
class SimulationCallingProfile(Base):
    """Calling Profile configuration within a simulation setting.

    Stores the QMS calling profile (FIFO/Overflow) and its serialized
    calling order JSON so it can be reloaded via "Load History".
    """
    __tablename__ = "simulation_calling_profiles"
    ...
    default_category = Column(String(255), nullable=False)
    profile_config_json = Column(Text, nullable=False)

    setting = relationship("SimulationSetting", back_populates="calling_profiles")


class SimulationCounter(Base):
    """Teller counter configuration within a simulation setting.

    Stores which calling profiles and operator profiles are assigned
    to each physical counter.
    """
    ...
    counter_profiles_json = Column(Text, nullable=False)
    operator_profiles_json = Column(Text, nullable=False)

    setting = relationship("SimulationSetting", back_populates="counters")
...
def init_db():
    ...
    db = LocalSession()
    try:
        db.execute(text("ALTER TABLE ingested_files ADD file_mtime FLOAT;"))
        db.commit()
    except Exception:
        db.rollback()
    try:
        db.execute(text("ALTER TABLE simulation_settings ADD resolution_mode VARCHAR(50);"))
        db.commit()
        print("Migrated: added resolution_mode to simulation_settings")
    except Exception:
        db.rollback()
    try:
        db.execute(text("ALTER TABLE simulation_settings ADD category_max_wait_times_json TEXT;"))
        db.commit()
        print("Migrated: added category_max_wait_times_json to simulation_settings")
    except Exception:
        db.rollback()
    finally:
        db.close()
...
def get_weekly_pattern_from_db(branch_id, category_id=0, month=None, year=None):
    """
    Get weekly pattern grouped by day of week, using the same data source as validation.
    ...
    validation = get_validation_data_from_db(branch_id, category_id, month, year)
    comparison = validation.get("comparisonData") if validation else None
    if not comparison:
    ...
    for item in comparison:
        ...
        day_totals[dow].append(item["predicted"])
...
def save_simulation_setting(config_dict: dict) -> int:
    ...
    try:
        ...
        cat_wait_times = config_dict.get("category_max_wait_times")
        cat_wait_times_json = (
            json.dumps(cat_wait_times) if cat_wait_times is not None else None
        )
        ...
        setting = SimulationSetting(
            ...
            resolution_mode=config_dict.get("resolution_mode", "hybrid"),
            category_max_wait_times_json=cat_wait_times_json,
        )
        db.add(setting)
        ...
        # Save QMS calling profiles
        for cp in config_dict.get("calling_profiles", []):
            order_data = cp.get("CALL", {}).get("order", []) if "CALL" in cp else cp.get("order", [])
            db.add(SimulationCallingProfile(
                setting_id=setting.id,
                profile_id=int(cp.get("id", cp.get("profile_id", 0))),
                name=cp.get("name", f"Profile {cp.get('id', '')}"),
                type=cp.get("type", "FIFO"),
                default_category=str(cp.get("default_category", "")),
                profile_config_json=json.dumps(order_data),
            ))

        # Save QMS counters
        for c in config_dict.get("counters", []):
            db.add(SimulationCounter(
                setting_id=setting.id,
                name=c.get("name", f"Counter {c.get('counter_id', '')}"),
                counter_id=int(c["counter_id"]),
                operator_name=c.get("operator_name"),
                counter_profiles_json=json.dumps(c.get("counter_profiles", [])),
                operator_profiles_json=json.dumps(c.get("operator_profiles", [])),
            ))
...
def get_simulation_setting_by_id(setting_id: int) -> dict:
    ...
    (services, hourly inflows, workgroups, skills, calling profiles, counters).
    ...
    try:
        ...
        cat_wait_times = None
        if setting.category_max_wait_times_json:
            try:
                cat_wait_times = json.loads(setting.category_max_wait_times_json)
            except Exception:
                pass
        ...
        return {
            ...
            "resolution_mode": setting.resolution_mode,
            "category_max_wait_times": cat_wait_times,
            ...
            "calling_profiles": [
                {
                    ...
                    "name": cp.name,
                    "type": cp.type,
                    "default_category": cp.default_category,
                    "CALL": {
                        "order": json.loads(cp.profile_config_json) if cp.profile_config_json else []
                    },
                }
                for cp in setting.calling_profiles
            ],
            "counters": [
                {
                    "name": c.name,
                    ...
                    "counter_profiles": json.loads(c.counter_profiles_json) if c.counter_profiles_json else [],
                    "operator_profiles": json.loads(c.operator_profiles_json) if c.operator_profiles_json else [],
                }
                for c in setting.counters
            ],
        }
    finally:
        db.close()
